#!/usr/bin/env python3
"""
generate_and_send.py — COMPLETE script with deduplication fix

- Filters out Q/A / mains practice (and blacklists insightsonindia by default)
- Deduplicates repeated paragraphs & bullets across all fields
- Uses Groq/OpenAI when available, with offline fallback and optional web enrichment
- Builds a grouped PDF and optionally emails it
- Test mode: --test-url (creates TEST PDF and does not send email)
"""
...
BLACKLIST_DOMAINS = [
    "insightsonindia.com",  # reference-only by your instruction; remove if you want included
]
...
# ---------------- Deduplication helpers (CRITICAL fix) ----------------
def dedupe_paragraphs(pars):
    """
    Normalize and dedupe paragraphs.
    Input: list or string. Output: list of deduped paragraphs (original casing).
    Keeps order and first occurrence.
    """
    if not pars:
        return []
    if isinstance(pars, str):
        items = [p.strip() for p in re.split(r'\n{1,}|(?<=\.)\s{2,}', pars) if p.strip()]
    else:
        items = [p.strip() for p in pars if p and p.strip()]
...
QUESTION_KEYWORDS = [
    ...
    "secure", "mains practice", "answer must", "model answer", "practice question",
    "mains answer", "questions and answers", "answer writing practice"
]
...
- Provide policy/scheme/act mentions in 'policy_points'.
- Provide 'detailed_brief' 120-220 words synthesizing causes and implications.
- Do NOT invent facts.
...
def make_detailed_brief(title, about, facts, policies):
    parts = []
    if about: parts.append(about)
    parts.extend(facts[:3])
    if policies: parts.append(policies[0])
    para = " ".join(parts)
...
def web_enrich(title, text):
    ...
    for r in results:
        ...
        wf = extract_facts(ptext, max_b=6); wp = extract_policy_points(ptext, max_n=6)
        for f in wf:
...
def process_article(title, url, text, img_bytes):
    ...
    for s in subs:
        if isinstance(s, dict):
            ...
            h = ""
            pts = s if isinstance(s, list) else []
        pts = dedupe_paragraphs(pts)
        normalized_subs.append({"heading": h, "points": pts})
    if parsed["policy_points"]:
        normalized_subs.append({"heading": "Key Provisions / Policy Mentions", "points": parsed["policy_points"]})
    parsed["sub_sections"] = normalized_subs

    # detailed brief
    dbrief = parsed.get("detailed_brief") or ""
    if isinstance(dbrief, list):
        dbrief = " ".join(dbrief)
    dbrief = dbrief.strip()
    if len(dbrief.split()) < 100:
        pieces = []
        if parsed.get("about"):
            pieces.append(parsed["about"])
        pieces.extend(parsed.get("facts_and_policies", [])[:3])
        if parsed.get("policy_points"):
            pieces.append(parsed["policy_points"][0])
        dbrief = " ".join(pieces)
    parsed["detailed_brief"] = dedupe_sentences_in_text(dbrief)
    ...
    if not impact:
        impact = make_impact(text)
    parsed["impact_or_analysis"] = dedupe_paragraphs(impact)
    ...
    if need_web and (SERPAPI_KEY or BING_API_KEY):
        try:
            ...
            parsed["facts_and_policies"] = dedupe_paragraphs(merged_facts)[:8]
            parsed["policy_points"] = dedupe_paragraphs(merged_policies)[:8]
            if parsed.get("policy_points"):
